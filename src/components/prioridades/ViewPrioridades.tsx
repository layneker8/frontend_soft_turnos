...
const ViewPrioridades: React.FC = () => {
	const {
		prioridades,
		loading,
		saving,
	...
	const [selected, setSelected] = useState<FullPrioridad | null>(null);
	const [modalMode, setModalMode] = useState(false); // false for edit/create, true for view
	const [searchTerm, setSearchTerm] = useState("");
	const [showDeleteModal, setShowDeleteModal] = useState(false);
	const [prioridadToDelete, setPrioridadToDelete] =
		useState<FullPrioridad | null>(null);
	...
	const filtered = prioridades.filter(
		(p) =>
	...
	const handleView = useCallback(
		async (p: FullPrioridad) => {
			const full = await getPrioridadById(p.id);
			if (full) {
	...
	const handleDelete = useCallback((p: FullPrioridad) => {
		setPrioridadToDelete(p);
		setShowDeleteModal(true);
	}, []);

	const confirmDelete = useCallback(async () => {
		if (!prioridadToDelete) return;
		const res = await deletePrioridad(prioridadToDelete.id);
		if (res.ok) {
			setShowDeleteModal(false);
			setPrioridadToDelete(null);
		}
	}, [prioridadToDelete, deletePrioridad]);

	const columns: TableColumn<FullPrioridad>[] = useMemo(
		() => [
			{
				id: "nombre",
				name: "Nombre",
				selector: (r) => r.nombre_prioridad,
				sortable: true,
				wrap: true,
			},
			{
	...
	const handleModalSubmit = async (
		data: CreatePrioridadData | Omit<UpdatePrioridadData, "id">
	) => {
		setServerError(undefined);
		setServerFieldErrors(undefined);
		if (selected) {
			const payload = {
				id: selected.id,
				...(data as Omit<UpdatePrioridadData, "id">),
			} as UpdatePrioridadData;
			const res = await updatePrioridad(selected.id, payload);
			if (!res.ok) {
		...
		const res = await createPrioridad(data as CreatePrioridadData);
		if (!res.ok) {
			setServerError(res.message);
			setServerFieldErrors(res.fieldErrors);
		} else {
			...
			<div className="p-6">
				<div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
					<div className="flex items-center gap-2 text-red-800 dark:text-red-200">
						...
						<span>No tienes permisos para ver las prioridades</span>
					</div>
				</div>
			</div>
		);
	...
	return (
		<div className="p-6">
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
				<div>
					...
					<p className="text-gray-600 dark:text-gray-400">
						Administra niveles de prioridad del sistema
					</p>
				</div>
			...
			{showDeleteModal && prioridadToDelete && (
				<Modal
								...
								<div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
									...
									<div className="mt-2">
										<p className="text-sm text-gray-500 dark:text-gray-400">
											¿Estás seguro de que deseas eliminar la prioridad{" "}
											<strong>{prioridadToDelete.nombre_prioridad}</strong>?
...
export default ViewPrioridades;
